...
function markClick(e) {
  if (isIE || isOpera6) {
    winX=event.screenX +200;
    winY=event.screenY -200;
  }
  else if (isN4 || isN6) {
    winX=e.screenX;
    winY=e.screenY;
...
function showCalTab(frmName,txtField,id) {
	hiddenField = true;
	actualField = id;
	tempField = txtField;
	showCal(frmName,txtField);
}

function showCal(frmName,txtField) {

	alert("I am here");

  var lastCal=currentCal;
  var d=new Date(), hasCal=false;
  addCalendar("Calendar"+txtField, "Select Date", txtField, frmName);
  setWidth(90, 1, 15, 1);
  currentCal = findCalendar("Calendar"+txtField);
...
function pickDate(d) {
  ...
  {
	  setDateFromHidden(tempField, actualField);
  }
}
// ------
...
function isLesserThanSysdate(txt) {
  var dtSys = new Date();
  var dtComp = new Date(dtSys.getYear(), dtSys.getMonth(), dtSys.getDate(), 0, 0, 0, 0);
  var dtIn = new Date(txt);
  iIn = dtIn.valueOf();
  iComp = dtComp.valueOf();
  if (iIn > iComp)
    return false;
  else if (iIn == iComp)
    return false;
